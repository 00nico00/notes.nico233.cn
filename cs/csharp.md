...
为了优化 `GC` 性能，托管堆被分为了三代，以用来单独出来短生存期和长生存期对象。`GC` 主要是在回收短生存期对象时发生。首先，新对象会被存储在第 0 代中，如果未被回收，那么则被升级存储到第 1 代或者第 2 代当中。因为第一条注意事项 **“压缩托管堆的一部分内存比压缩整个托管堆快”** 因此 `GC` 在回收时只会回收释放特定级别的内存，而不是整个托管堆的内存。